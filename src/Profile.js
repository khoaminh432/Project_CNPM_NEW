...
const Profile = ({ userInfo, setUserInfo, isEditing, setIsEditing }) => {
  ...
  // --- SỬA LẠI: CHỈ LẤY HỌC SINH CỦA PHỤ HUYNH NÀY ---
  useEffect(() => {
    // Chỉ gọi khi đã có thông tin phụ huynh (parent_id)
    if (userInfo && userInfo.parent_id) {
        fetch('http://localhost:8081/api/students')
        .then(res => res.json())
        .then(data => {
            // Lọc danh sách: Chỉ lấy học sinh có parent_id trùng với người đang đăng nhập
            const myStudents = data.filter(s => s.parent_id === userInfo.parent_id);
            setStudents(myStudents);
        })
        .catch(err => console.error(err));
    }
  }, [userInfo.parent_id]); // Chạy lại khi parent_id thay đổi

  // Không cần gọi API profile ở đây nữa vì HomePH đã truyền xuống rồi
