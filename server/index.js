...
// 1. CAC API HE THONG (GIU NGUYEN KHONG DOI)
...
// 2. API QUAN TRONG: LAY TRANG THAI SINH VIEN (SUA LOGIC NGAY THANG)
app.get('/api/route-students-status', (req, res) => {
    const routeId = req.query.id;
    const parentId = req.query.parentId;
    const scheduleId = req.query.scheduleId;

    // Neu chua co scheduleId (xe chua chay), tra ve mac dinh
    if (!scheduleId) {
    ...
    // Neu co scheduleId, JOIN chinh xac vao chuyen do
    let sql = `
